import paddle
from model.get_model import get_swim, get_swimb, get_swiml, get_swimb_linear
from dataloader_linear import MyDateset
import os
import time
...
batch_s = 4
output_model_dir = './swimb_linear'
if not os.path.exists(output_model_dir):
    os.mkdir(output_model_dir)

# log file
file_path = './swimb_linear/train_log.txt'
f_log = open(file_path, 'a')

######## model
# model = get_unetrw()
# model = get_swiml()
model = get_swimb_linear()
# model = get_swimb()
model.train()
...
# 需要接续之前的模型重复训练可以取消注释
# param_dict = paddle.load('./swiml/model_36.pdparams')
...
max_epoch=10
scheduler = paddle.optimizer.lr.CosineAnnealingDecay(learning_rate=0.00008, T_max=max_epoch)
...
for epoch in range(start_epoch, max_epoch):
    ...
    for step, data in enumerate(train_dataloader):
        ...
        loss12 = (loss1 + loss2) * 0.5
        loss3 = paddle.nn.functional.square_error_cost(pre_8, label_8).mean()

        loss = loss12 + loss3 * 0.5
        ...
        if now_step%10==0:
            content_log = time.strftime('%Y-%m-%d %H:%M:%S',time.localtime(time.time())) + ' ' + "epoch: {}, batch: {}, loss1 is: {}, loss2 is: {}, lr:{}".format(epoch, step, loss12.mean().numpy(), loss3.mean().numpy(), lr_update)
            print(content_log)
